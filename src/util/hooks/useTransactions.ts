...
const useTransactions = () => {
    ...
    const allTransactions = pending
        ...
        .flatMap((t, _, transactions) => {
            ...
            if (t.metaType === MetaType.SPEEDING_UP) {
                const speedUpTx = transactions.find(
                    (tx) =>
                        tx.rawTransaction === t.rawTransaction &&
                        tx.metaType === MetaType.SPEED_UP
                )
            ...
            if (t.metaType === MetaType.CANCELLING) {
                const cancelTx = transactions.find(
                    (tx) =>
                        tx.rawTransaction === t.rawTransaction &&
                        tx.metaType === MetaType.CANCEL
                )
